resource "null_resource" "empty_inventory" {
  provisioner "local-exec" {
    command = "touch ./ansible/tmp/inventory-${var.base_name}.yml"
  }
}
...
resource "null_resource" "run_ansible_kube_cluster" {
  provisioner "local-exec" {
    command = <<EOT
        ...
        -i ./ansible/tmp/inventory-${var.base_name}.yml \
        --extra-vars "aws_ssh_user=${var.aws_ssh_user} pod_cidr=${var.pod_cidr} service_cidr=${var.service_cidr} \
         join_token=${data.template_file.kubeadm_token.rendered} cluster_name=${var.base_name} \
